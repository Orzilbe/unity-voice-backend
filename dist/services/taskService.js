 ...
 */
async function createTask(userId, topicName, level, taskType) {
    ...
    try {
        ...
        try {
            ...
            const [levelRows] = await connection.query('SELECT 1 FROM Levels WHERE TopicName = ? AND Level = ?', [topicName, levelStr]);
            ...
            const [existingTasks] = await connection.query(`SELECT TaskId FROM Tasks 
         WHERE UserId = ? 
         AND TopicName = ? 
         AND Level = ? 
         AND TaskType = ? 
         AND CompletionDate IS NULL`, [userId, topicName, levelStr, taskType]);
            if (Array.isArray(existingTasks) && existingTasks.length > 0) {
                console.log(`Found existing incomplete task: ${existingTasks[0].TaskId}`);
                await connection.commit();
                return existingTasks[0].TaskId;
            }
            // יצירת מזהה למשימה חדשה
            const taskId = (0, uuid_1.v4)();
            // יצירת משימה חדשה
            const [result] = await connection.query(`INSERT INTO Tasks 
         (TaskId, UserId, TopicName, Level, TaskType, TaskScore, CreatedAt, UpdatedAt)
         VALUES (?, ?, ?, ?, ?, 0, NOW(), NOW())`, [taskId, userId, topicName, levelStr, taskType]);
 ...
 */
async function completeTask(taskId, score, duration) {
    ...
    try {
        ...
        if (!taskDuration) {
            const [taskRows] = await pool.query('SELECT StartDate FROM Tasks WHERE TaskId = ?', [taskId]);
        ...
        // עדכון המשימה
        const [result] = await pool.query('UPDATE Tasks SET CompletionDate = ?, TaskScore = ?, DurationTask = ?, UpdatedAt = NOW() WHERE TaskId = ?', [completionDate, score, taskDuration || 0, taskId]);
 ...
 * @returns האם העדכון הצליח
 */
async function updateUserScore(taskId, score) {
    try {
        const pool = await (0, db_1.getDbPool)();
        if (!pool) {
            return false;
        ...
        // קבלת פרטי המשימה
        const [tasks] = await pool.query('SELECT UserId, TopicName, Level, TaskType FROM Tasks WHERE TaskId = ?', [taskId]);
        if (!Array.isArray(tasks) || tasks.length === 0) {
            return false;
        }
        const task = tasks[0];
        // עדכון הציון של המשתמש ברמה הספציפית
        await pool.query(`INSERT INTO UserINLevel (TopicName, Level, UserId, EarnedScore, CompletedAt) 
       VALUES (?, ?, ?, ?, NOW())
       ON DUPLICATE KEY UPDATE EarnedScore = ?, CompletedAt = NOW()`, [task.TopicName, task.Level, task.UserId, score, score]);
        // חישוב ועדכון הציון הכללי של המשתמש
        await pool.query(`UPDATE Users SET Score = (
         SELECT AVG(EarnedScore) 
         FROM UserINLevel 
         WHERE UserId = ?
       ) WHERE UserId = ?`, [task.UserId, task.UserId]);
        // בדיקה אם המשימה היא מסוג 'conversation'
        if (task.TaskType === Task_1.TaskType.CONVERSATION) {
            // 1. עדכון הרשומה הרלוונטית ב-UserINLevel
            await pool.query(`UPDATE UserINLevel 
         SET CompletedAt = NOW() 
         WHERE UserId = ? AND TopicName = ? AND Level = ? AND CompletedAt IS NULL`, [task.UserId, task.TopicName, task.Level]);
            // חישוב סכום הציונים מכל המשימות הרלוונטיות
            const [scoreResults] = await pool.query(`SELECT SUM(TaskScore) as TotalScore 
         FROM Tasks 
         WHERE UserId = ? AND TopicName = ? AND Level = ?`, [task.UserId, task.TopicName, task.Level]);
            const totalScore = scoreResults[0]?.TotalScore || 0;
            // עדכון EarnedScore בהתבסס על סכום הציונים
            await pool.query(`UPDATE UserINLevel 
         SET EarnedScore = ? 
         WHERE UserId = ? AND TopicName = ? AND Level = ?`, [totalScore, task.UserId, task.TopicName, task.Level]);
            // קבלת הרמה הנוכחית
            const currentLevel = parseInt(task.Level);
            const nextLevel = currentLevel + 1;
            // 2. הוספת רשומה חדשה לרמה הבאה
            await pool.query(`INSERT INTO UserINLevel (UserId, TopicName, Level, EarnedScore, CompletedAt) 
         VALUES (?, ?, ?, 0, NULL)
         ON DUPLICATE KEY UPDATE EarnedScore = EarnedScore`, [task.UserId, task.TopicName, nextLevel]);
 ...
 */
async function addWordsToTask(taskId, wordIds) {
    ...
    try {
      ...
      `);
        ...
        for (const wordId of wordIds) {
            try {
                await pool.query('INSERT IGNORE INTO WordsInTask (TaskId, WordId) VALUES (?, ?)', [taskId, wordId]);
            }
            catch (error) {
 ...
 */
async function getUserTasks(userId, topicName) {
