...
exports.analyzeConversationResponse = void 0;
...
function initializeOpenAI() {
    ...
    try {
        const openaiInstance = new openai_1.AzureOpenAI({
...
const analyzeConversationResponse = async (req, res) => {
    try {
        ...
        const { text, topicName, level, previousMessages = [] } = req.body;
        if (!text || !topicName) {
        ...
        const systemPrompt = generateConversationAnalysisPrompt(user.EnglishLevel, topicName, level, learnedWords, requiredWords);
        // Prepare messages
        const messages = [
...
exports.analyzeConversationResponse = analyzeConversationResponse;
// Helper function to get learned words for topic and level
async function getLearnedWordsForTopicAndLevel(userId, topicName, level) {
    try {
...
async function getRequiredWordsForTopicAndLevel(topicName, level, englishLevel) {
    try {
        // Get words that match the topic, level, and English proficiency
        const words = await Word_1.default.findByTopicAndLevel(topicName, englishLevel);
...
function generateConversationAnalysisPrompt(englishLevel, topicName, level, learnedWords, requiredWords) {
...
function analyzeWordUsage(text, requiredWords) {
    ...
    return requiredWords.map(word => {
        ...
        if (used) {
            ...
            const containingSentence = sentences.find(sentence => sentence.toLowerCase().includes(wordLower));
...
function generateNextQuestion(topicName, englishLevel) {
