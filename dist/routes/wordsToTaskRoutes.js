...
router.post('/to-task', authMiddleware_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const [taskRows] = await connection.query('SELECT TaskId, TopicName, Level FROM Tasks WHERE TaskId = ? AND UserId = ?', [taskId, userId]);
            ...
            const success = await (0, taskService_1.addWordsToTask)(taskId, wordIds);
            if (success) {
                console.log(`✅ Successfully added ${wordIds.length} words to task ${taskId}`);
                // Optional: Get updated word count for confirmation
                const [countResult] = await connection.query('SELECT COUNT(*) as wordCount FROM wordintask WHERE TaskId = ?', [taskId]);
                const totalWordsInTask = countResult[0]?.wordCount || 0;
...
router.get('/in-task', authMiddleware_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        try {
            // Verify task belongs to user
            const [taskCheck] = await connection.query('SELECT TaskId FROM Tasks WHERE TaskId = ? AND UserId = ?', [taskId, userId]);
