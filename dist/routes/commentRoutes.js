...
router.post('/', authMiddleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        const { CommentID, PostID, commentContent, Feedback, requiredWords = [], postContent = '', durationTask = 0, taskId // 🔥 הוסף את זה
         } = req.body;
 ...
 */
function generateDetailedFeedback(commentContent, requiredWords, postContent) {
    ...
    if (postContent) {
        ...
        if (hasQuestions) {
            const addressesQuestions = commentContent.toLowerCase().includes('think') ||
        ...
        const relevantWords = topicWords.filter(word => commentContent.toLowerCase().includes(word.toLowerCase()));
 ...
 */
function extractTopicWords(postContent) {
 ...
 */
router.post('/test-feedback', authMiddleware_1.authMiddleware, async (req, res) => {
    try {
...
router.post('/', authMiddleware_1.authMiddleware, async (req, res) => {
    try {
        console.log('Creating comment with data:', req.body);
        const { CommentID, PostID, commentContent, Feedback, requiredWords = [], postContent = '', durationTask = 0 } = req.body;
        if (!PostID) {
            return res.status(400).json({
                success: false,
                error: 'PostID is required'
...
router.get('/', authMiddleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        try {
            if (postId) {
                const [comments] = await connection.execute('SELECT * FROM comments WHERE PostID = ? ORDER BY CommentID DESC', [postId]);
