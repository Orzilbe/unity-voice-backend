...
router.post('/', authMiddleware_1.authMiddleware, async (req, res) => {
    try {
        ...
        const { CommentID, PostID, commentContent, Feedback, requiredWords = [], // ← הוסף את זה
...
function generateCommentFeedback(commentContent, requiredWords) {
    const wordCount = commentContent.split(' ').length;
    const usedRequiredWords = requiredWords.filter(word => commentContent.toLowerCase().includes(word.toLowerCase()));
    ...
    const feedback = {
        ...
        message: usedRequiredWords.length === requiredWords.length
            ...
            : requiredWords.length > 0
                ...
                : "✅ Great comment!",
        suggestions: requiredWords.filter(word => !commentContent.toLowerCase().includes(word.toLowerCase()))
    };
