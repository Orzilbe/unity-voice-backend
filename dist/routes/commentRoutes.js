...
router.post('/', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        ...
        const { taskId, commentContent, requiredWords = [], postContent = '', durationTask = 0 } = req.body;
        ...
        const feedback = generateDetailedFeedback(commentContent, postContent, requiredWords, task.TopicName);
        // Create comment
        ...
        if (userId && durationTask !== undefined) {
            try {
                await Task_1.default.completeTask(taskId, feedback.totalScore, durationTask);
...
router.get('/:taskId', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        ...
        const comments = await Comment_1.default.findByTaskId(taskId);
 ...
 */
router.post('/test-feedback', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        ...
        // Validate comment
        const validation = await validateComment(commentContent, postContent || '', 'test-task-id', requiredWords || []);
        if (!validation.isValid) {
            return res.json({
                success: false,
        ...
        // Generate feedback
        const feedback = generateDetailedFeedback(commentContent, postContent || '', requiredWords || [], topicName || 'general');
        return res.json({
            success: true,
            feedback: feedback,
            validation: validation,
 ...
 */
async function validateComment(commentContent, postContent, taskId, requiredWords) {
    const issues = [];
    const details = {};
    // Basic length check
 ...
 */
function findMostCommonWord(words) {
    const frequency = {};
 ...
 */
function generateDetailedFeedback(commentContent, postContent, requiredWords, topicName) {
    ...
    const hasConnectors = connectingWords.some(word => commentContent.toLowerCase().includes(word));
    if (hasConnectors)
        clarityScore += 15;
    // 2. Grammar Score (0-100)
    let grammarScore = 30; // Base score
    // Capitalization
    const properlyCapitalized = sentences.filter(s => s.trim().length > 0 && /^[A-Z]/.test(s.trim())).length;
    ...
    if (!/\b(me are|me is|me have|i are)\b/i.test(commentContent))
        grammarScore += 15;
    ...
    if (uniqueWords.size / wordCount > 0.8)
        vocabularyScore += 10;
    ...
    const mentionedKeywords = topicKeywords.filter(keyword => commentContent.toLowerCase().includes(keyword.toLowerCase()));
    ...
    const hasPersonalTouch = personalIndicators.some(indicator => commentContent.toLowerCase().includes(indicator));
    if (hasPersonalTouch)
        contentRelevanceScore += 10;
 ...
 */
function extractTopicKeywords(topicName) {
