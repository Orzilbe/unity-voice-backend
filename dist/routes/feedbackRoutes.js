 ...
 */
router.post('/comment', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { commentContent, postContent, requiredWords, topicName, level } = req.body;
        ...
        const fallbackFeedback = generateSimpleFeedback(req.body.commentContent || '', req.body.requiredWords || []);
        res.json({
            ...fallbackFeedback,
 ...
 */
function createFeedbackPrompt(commentContent, postContent, requiredWords, topicName, level) {
 ...
 */
function validateAndNormalizeFeedback(aiResult, requiredWords, commentContent) {
 ...
 */
function parseTextualFeedback(responseText, requiredWords, commentContent) {
 ...
 */
function generateSimpleFeedback(commentContent, requiredWords) {
