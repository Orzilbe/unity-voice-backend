...
const express_1 = __importDefault(require("express")); // ← תיקנתי את הרווח
...
router.post('/generate', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const { topicName, englishLevel, learnedWords, requiredWords } = req.body;
 ...
 */
router.post('/create/:topicName', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
...
async function generatePostWithAI(topicName, englishLevel, requiredWords) {
    try {
...
function createTopicPrompt(topicName, englishLevel, requiredWords) {
...
function validatePostContent(postContent, topicName, requiredWords) {
    const issues = [];
    const suggestions = [];
    const lowerContent = postContent.toLowerCase();
    const lowerTopic = topicName.toLowerCase();
    // בדיקה שכל המילים הנדרשות מופיעות
    const missingWords = requiredWords.filter(word => !lowerContent.includes(word.toLowerCase()));
    ...
    if (lowerTopic.includes('innovation') || lowerTopic.includes('technology')) {
        const hasEnvironmentWords = ['wind', 'solar', 'environmental', 'sustainability', 'green', 'eco'].some(word => lowerContent.includes(word));
        ...
        const hasTechWords = ['technology', 'innovation', 'startup', 'ai', 'cyber', 'research', 'development'].some(word => lowerContent.includes(word));
        if (!hasTechWords) {
    ...
    if (lowerTopic.includes('environment') || lowerTopic.includes('sustainability')) {
        const hasTechWords = ['startup', 'innovation', 'ai', 'cyber'].some(word => lowerContent.includes(word) && !lowerContent.includes('green ' + word));
        if (hasTechWords) {
    ...
    if (lowerTopic.includes('diplomacy')) {
        const hasNonDiplomaticWords = ['startup', 'technology', 'environment'].some(word => lowerContent.includes(word));
...
async function generatePostWithAIImproved(topicName, englishLevel, requiredWords) {
    let attempts = 0;
    const maxAttempts = 3;
...
function createFallbackPost(topicName) {
    const formattedTopic = topicName
        .split('-')
