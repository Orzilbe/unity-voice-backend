...
import { getPostForTask } from '../services/postGenerationService';
...
router.get('/:taskId', authenticateToken, async (req: IUserRequest, res: Response) => {
  try {
    ...
    const postResult = await getPostForTask(
      taskId,
      userId,
      task.TopicName,
      await getUserEnglishLevel(userId)
    );
 ...
 */
router.post('/regenerate/:taskId', authenticateToken, async (req: IUserRequest, res: Response) => {
  try {
    const { taskId } = req.params;
    const userId = req.user?.id || req.user?.userId;
    ...
    const cleared = await Task.clearPostLink(taskId);
    if (!cleared) {
 ...
 */
router.get('/stats/:userId', authenticateToken, async (req: IUserRequest, res: Response) => {
  try {
    ...
    const topics = topicStats as any[];
