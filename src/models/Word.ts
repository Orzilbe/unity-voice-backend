// unity-voice-backend/src/models/Word.ts
...
class Word {
  ...
  static async findByTopicAndLevel(topicName: string, level: string): Promise<IWord[]> {
    try {
      const [rows] = await pool.execute<IWord[]>(
        'SELECT * FROM Words WHERE TopicName = ? AND EnglishLevel = ?',
        [topicName, level]
  ...
  static async findByTopic(topicName: string): Promise<IWord[]> {
    try {
      const [rows] = await pool.execute<IWord[]>(
        'SELECT * FROM Words WHERE TopicName = ?',
        [topicName]
      );
      return rows;
    } catch (error) {
