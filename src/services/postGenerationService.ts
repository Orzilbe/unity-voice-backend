...
export interface PostGenerationResult {
  ...
  source: 'existing' | 'reused' | 'generated' | 'fallback';
 ...
 */
export async function getPostForTask(
  taskId: string,
  userId: string,
  topicName: string,
  ...
  try {
    ...
    const task = await Task.findByIdWithPost(taskId);
    if (!task) {
 ...
 */
async function findAndLinkReusablePost(
  taskId: string,
  userId: string,
  topicName: string
): Promise<PostGenerationResult | null> {
  try {
    const reusablePosts = await Post.findReusablePosts(userId, topicName, 3);
    ...
    const linkSuccess = await Task.linkPost(taskId, selectedPost.PostID);
    if (!linkSuccess) {
 ...
 */
async function generateAndLinkNewPost(
  taskId: string,
  userId: string,
  topicName: string,
  englishLevel: string
  ...
  try {
    ...
    // Link post to task
    const linkSuccess = await Task.linkPost(taskId, postId);
    if (!linkSuccess) {
 ...
 */
async function generatePostContentWithOpenAI(
  topicName: string,
  englishLevel: string,
  requiredWords: string[]
): Promise<string> {
 ...
 */
function createEnhancedTopicPrompt(
  topicName: string,
  englishLevel: string,
  requiredWords: string[]
): string {
 ...
 */
function getTopicFocusInstructions(topicName: string): string {
 ...
 */
async function shouldReuseExistingPost(userId: string, topicName: string): Promise<boolean> {
  try {
    ...
    const postCount = (rows as any)[0]?.post_count || 0;
    
    // Only reuse if user has seen fewer than 2 posts for this topic
    if (postCount < 2) {
      const reusablePosts = await Post.findReusablePosts(userId, topicName, 1);
 ...
 */
async function createAndLinkFallbackPost(
  taskId: string,
  userId: string,
  topicName: string
): Promise<PostGenerationResult> {
  ...
  try {
    ...
    await Task.linkPost(taskId, postId);
...
function createFallbackContent(topicName: string, requiredWords: string[]): string {
